...
using DifferentialEquations, Plots, PlutoUI, JLD2, PolynomialRoots, Random, Measures

# ╔═╡ 70fa3085-357a-4905-adaf-ca01376d5e78
md"""
# Example 2: Bogdanov Takens with cubic terms

$\dot{x} = y$

$\dot{y} = \mu_1 + \mu_2 x - \delta xy + x^2 - x^2y$
...
# ╔═╡ cac2fef5-5952-4422-8dc2-4df8d921a047
md"""
# Figure 5
"""
...
begin
	file_temp = load("homoclinic_curve.jld2")
...
begin
	...
	μh1 = -0.5:0.01:0
	μh2 = -0.5:0.01:0.8
	plta = plot(μsn,sn1.(μsn),c=:black,label="Saddle Node",xlabel="μ2",ylabel="μ1",title="δ=1.5",margin=5mm,legendfontsize=10)
	...
	plot!(plta,hom[12][2,:],hom[12][1,:],c=:gray,ls=:dashdot,label="Homoclinic")
	scatter!(plta,[0],[0],c=:gray,ms=5,label="BT")
	scatter!(plta,[bt2],[sn2(bt2)],c=:gray,ms=5,label="")
	#scatter!([μ20-dμ2],[sn1(μ20)],c=:blue,label="",xlims=(-0.4,0.3),ylims=(-0.3,0.05))
	scatter!(plta,[0,-0.3],[-0.15,-0.005],ms=5,c=:blue,label="η=0")
	scatter!(plta,[0,-0.3],[-0.145,0.005],ms=5,c=:red,label="η=0.02",xlims=(-0.4,0.3),ylims=(-0.3,0.05))
	pltb = plot(μsn,sn1.(μsn),c=:black,label="Saddle Node",xlabel="μ2",ylabel="μ1",title="δ=0.5",margin=5mm,legendfontsize=10)
	...
	plot!(pltb,hom[5][2,:],hom[5][1,:],c=:gray,ls=:dashdot,label="Homoclinic")
	scatter!(pltb,[0],[0],c=:gray,ms=5,label="BT")
	scatter!(pltb,[bt2],[sn2(bt2)],c=:gray,ms=3,label="")
	#scatter!([μ20-dμ2],[sn1(μ20)],c=:blue,label="",xlims=(-0.4,0.3),ylims=(-0.3,0.05))
	scatter!(pltb,[0],[-0.11],ms=5,c=:blue,label="η=0")
	scatter!(pltb,[0],[-0.145],ms=5,c=:red,label="η=0.02",xlims=(-0.4,0.3),ylims=(-0.3,0.05))
	plt_ab = plot(pltb,plta,layout=(1,2),size=(1200,500))
	savefig(plt_ab, "fig6.svg")
	plt_ab
end	

# ╔═╡ 1f9c7d7c-1454-47b6-bb3e-370f02383454
md"""
# Figure 4
Three phase portraits without noise
"""
...
# ╔═╡ 742b677d-e78d-4d55-bbf4-7323dabdf2f0
md"""
# Figure 6
Three phase portraits with noise
"""
...
manifest_format = "2.0"
project_hash = "b72823c93ce870df80f06a138ae96bab0dbf33fa"
